...
public class XmlDocument {
    ...
    public XmlDocument(Element doc){
        this.root = new DocumentImpl();
        this.root.appendChild(Util.importNode(this.root,doc,true));
    }
...
//				Voglio tutto su una riga, quindi ignoro il ritorno a capo
   ...
   public XmlDocument(InputStream xml){
    ...
    private static void writeXML(Document d, OutputStream os) throws IOException {
	    ...
	    if(node.hasChildNodes()) {
		    ...
		    for(int i=0;i<text.getData().length();i++) {
			    ...
			    if(buf.length()>0 && buf.charAt(buf.length()-1)!=' ') {
		        	...
		        	char cTmp=text.getData().charAt(i);
		        	if (cTmp=='&')
		        		buf.append("&amp;");
		        	else
		        		buf.append(cTmp);
